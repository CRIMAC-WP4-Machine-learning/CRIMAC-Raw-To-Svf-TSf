...
parser = argparse.ArgumentParser(
    description="Read Examples raw file and output json data file for a single ping."
)
parser.add_argument(
    "--channel",
    metavar="Channel",
...
parser.add_argument(
    "--pingNo", metavar="PingNo", type=int, help="Examples ping nummer "
)
parser.add_argument(
    "--inputFile",
    metavar="File",
...
for j in range(0, len(cal_fm_freqs)):
    ...
    angle_offset_athwartship.append(
        calibration_fm[cal_fm_freqs[j]]["angle_offset_athwartship"]
    )
    angle_offset_alongship.append(
        calibration_fm[cal_fm_freqs[j]]["angle_offset_alongship"]
    )
    beam_width_athwartship.append(
        calibration_fm[cal_fm_freqs[j]]["beam_width_athwartship"]
    )
...
GainNom = np.ndarray.tolist(
    np.array(calibration_dict["gain"][(len(calibration_dict["gain"]) - 1)])
)  # Correct now, but check again
# EquivalentBeamAngle=np.ndarray.tolist(np.array(calibration.equivalent_beam_angle[PingNo]))
EquivalentBeamAngle = np.ndarray.tolist(
    np.array(calibration_dict["equivalent_beam_angle"])
)
...
sampleOffset = np.ndarray.tolist(
    np.array(raw_data.sample_offset[PingNo])
)  # Sample offset for PingNo
...
data = {
    ...
    "FIL1": {
        "1": {
            ...
            "coefficients": {"real": filt1_real, "imag": filt1_imag},
        },
        "2": {
            "noOfCoefficients": noOfCoefficients2,
            "decimationFactor": decimationFactor2,
            "coefficients": {"real": filt2_real, "imag": filt2_imag},
        },
    },
    "RAW3": {
        "offset": sampleOffset,
        ...
        "quadrant_signals": {
            "Q1": {"real": Q1_real, "imag": Q1_imag},
            "Q2": {"real": Q2_real, "imag": Q2_imag},
