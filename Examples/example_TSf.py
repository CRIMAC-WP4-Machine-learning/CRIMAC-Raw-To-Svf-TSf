...
if __name__ == '__main__':
    ...
    gamma_theta = Calculation.calcGamma(angle_sensitivity_alongship_fnom, f_c, fnom)
    gamma_phi = Calculation.calcGamma(angle_sensitivity_athwartship_fnom, f_c, fnom)

    # Calculate the physical angles
    theta_n, phi_n = Calculation.calcAngles(y_pc_halves_n, gamma_theta, gamma_phi)
    ...
    r_n, _ = Calculation.calcRange(sampleInterval, sampleCount, c, offset)

    alpha_f_c = Calculation.calcAbsorption(temperature, salinity, depth, acidity, c, f_c)
    ...
    Y_pc_t_m, Y_mf_auto_red_m, Y_tilde_pc_t_m, f_m_t = \
        Calculation.calcDFTforTS(y_pc_t_n, y_mf_auto_red_n, n_f_points, f0, f1, f_s_dec)

    # Calculate the power by frequency from a single target
    P_rx_e_t_m = Calculation.calcPowerFreq(N_u, Y_tilde_pc_t_m, z_td_e, z_rx_e)
