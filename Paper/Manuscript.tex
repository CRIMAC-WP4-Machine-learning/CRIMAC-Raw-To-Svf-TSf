...
\caption{\label{Fig_TS_echogram} Sv as a function of ping number and range ($n$) for a calibration sphere. The sphere is located at approximately 6 m range. The red vertical line indicates the ping that is used for illustraing the $\ts(\freqsym)$ processing.}
\end{figure}
...
As a metallic sphere is a rather simple and ideal scatterer suitable for investigating TS of single scatterers, we use $\sv$ from a school of non-swimbladdered fish (Fig.~\ref{Fig_Sv_echogram}) collected with a 120 kHz centre frequency transducer to illustrate the processes to obtain $\sv(m)$. 
...
\caption{\label{Fig_Sv_echogram} Sv as a function of ping number and range range ($\samplesymt$) for a echogram raw data file. The upper yellow area shows the transmit pulse, a fish school is seen as a registration between 15 and 35 m range, and the sea flor is seen at approximately 50 m. The red vertical line indicates the ping that is used for illustraing the $\sv(\freqsym)$ processing.}
\end{figure}
